...
function Downloads() {
  ...
  // chart
    ...
    // state for store all job roles
    const [label,setLabel]=useState([])
    // state for all downloads count
    const [value,setValue] = useState([])

    // color picker

    const colorPallete=['#0a698c','#19e162','#ef36b4','#ddc11f','#dd3507','#bcd8dd']
    ...
    <div>
      <div className='container my-5'>
        <div className="d-flex justify-content-between align-items-center">
          ...
          <button onClick={handleOpen} style={{background:'#714a2f'}} className='btn text-light'>View in Chart</button>

        </div>
        <p className='my-3 fw-bolder'>Total Dowloaded resumes from our site is <span
        className='text-danger fs-4'>{downloadList.length}</span></p>
        ...
        
        {
          ...
          <div style={{height:'400px'}}className="shadow p-3 rounded">
            ...
            <div className="mt-3 text-center">
              <Link to={`/resume/${resume.id}/view` }><img className='w-100' src={resume.resumeIMG} height={'300px'} alt="Download cv"/></Link>
            </div>
          </div>
        </div>
